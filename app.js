...
function clearSelection() { selectedPlaces = []; updateSelectionUI(); updateAllMarkers(); }
...
window.generateItinerary = function() {
  ...
  selectedPlaces.forEach(function(id) {
    ...
    if (lugar) {
      if (lugar.coords && lugar.coords.length >= 2) paradas.push(lugar.coords[0] + "," + lugar.coords[1]); 
      else if (lugar.lat && lugar.lng) paradas.push(lugar.lat + "," + lugar.lng); 
    }
  });
